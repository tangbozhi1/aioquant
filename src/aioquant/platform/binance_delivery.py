...
class BinanceTrade(ExchangeGateway):
    ...
    def __init__(self, **kwargs):
        ...
        if not kwargs.get("host"):
            kwargs["host"] = "https://dapi.binance.com"
        if not kwargs.get("wss"):
            kwargs["wss"] = "wss://dstream.binance.com"
    ...
    async def _get_listen_key(self):
        while True:
            try:
                key = await self._rest_api.dapiPrivatePostListenKey()
    ...
    async def _reset_listen_key(self):
        ...
        while True:
            try:
                await self._rest_api.dapiPrivatePutListenkey(self._listen_key)
    ...
    async def connected_callback(self):
        if any(self.is_authenticated_channel(chan) for chan in self.websocket_channels.keys()):
            ...
            subscription = [chan for chan in raw_channels]
            streams = []
            for chan in subscription:
                ...
                if chan == self.std_channel_to_raw_channel(const.FILLS):
                    continue
                if chan == self.std_channel_to_raw_channel(const.CANDLES):
                    continue
                if chan == self.std_channel_to_raw_channel(const.TRADES):
                    continue
                if chan == self.std_channel_to_raw_channel(const.L2_BOOK):
                    continue
                if chan == self.std_channel_to_raw_channel(const.TICKER):
                    continue

                streams.append(f"{self._listen_key}@{chan}")
            ...
            SingleTask.run(self._init_callback, True) # 后面不是订阅，是无限循环请求
            await self._request_account(data)

    # 无限循环请求
    async def _request_account(self, data):
        while True:
            try:
                await asyncio.sleep(2)  # 请求限制
    ...
    async def process(self, msg):
        ...
        if 'e' in msg:
            ...
            for data in msg['result']:
                if data.get('res').get("balances"):
                    for asset in data.get('res').get("balances"):
                        await self._update_balance(asset)

                elif data.get('res').get("positions"):
                    for position in data.get('res').get("positions"):
    ...
    #     for data in info:
    #         update = False
    #         position = Position(self._platform, self._account, self._strategy, self.raw_symbol_to_std_symbol(data['symbol']))
    ...
    async def _update_balance(self, msg: dict):
        """
        {
            'asset': 'AAVE',
            'balance': '0.00000000',
        ...
        """
        assets = {}
        total = Decimal(msg['crossWalletBalance'])
        free = Decimal(msg['availableBalance'])
        locked = total - free
        assets[msg['asset']] = {
            "total": "%.8f" % total,
            "free": "%.8f" % free,
    ...
    async def _update_position(self, data):
        ...
        if Decimal(data['positionAmt']) > 0:
            if position.long_quantity != data['positionAmt']:
                update = True
                position.update(
                    margin_mode=MARGIN_MODE_FIXED if data['marginType'] == "ISOLATED" else MARGIN_MODE_CROSSED,
                    long_quantity=data['positionAmt'], long_avg_qty=data['positionAmt'],
                    long_open_price=data['entryPrice'],
                    long_hold_price=data['entryPrice'],
                    long_liquid_price = data['liquidationPrice'],
                    long_unrealised_pnl=data['unRealizedProfit'],
                    long_leverage=data['leverage'],
                    long_margin=data['isolatedMargin'])
        elif Decimal(data['positionAmt']) < 0:
            if position.short_quantity != data['positionAmt']:
                update = True
                position.update(
                    margin_mode=MARGIN_MODE_FIXED if data['marginType'] == "ISOLATED" else MARGIN_MODE_CROSSED,
                    short_quantity=abs(float(data['positionAmt'])), short_avg_qty=data['positionAmt'],
                    short_open_price=data['entryPrice'],
                    short_hold_price=data['entryPrice'],
                    short_liquid_price=data['liquidationPrice'],
                    short_unrealised_pnl=data['unRealizedProfit'],
                    short_leverage=data['leverage'],
                    short_margin=data['isolatedMargin'])
        elif Decimal(data['positionAmt']) == 0 and data['positionSide'] == 'BOTH':
            if position.long_quantity != 0 or position.short_quantity != 0:
                update = True
                position.update()
        ...
        self._position[self.raw_symbol_to_std_symbol(data['symbol'])] = position
        SingleTask.run(self._on_position_update_callback, position)
...
class BinanceMarket(ExchangeWebsocket):
    ...
    async def _init_rest(self):
        ...
        # 私有与公共的听唯一区别是如果有订阅任何私有频道必须要有listen_key,address只是多?listenKey=<validateListenKey>
        await self._get_listen_key()
        self._wsp = Websocket(self._wss + '/ws/' + self._listen_key, self.private_connected_callback, process_callback=self.process_private)
        LoopRunTask.register(self._reset_listen_key, 60 * 30)
    ...
    async def _address(self):
        ...
        for chan in subscription:
            ...
            # 私有在公共线路无用，过滤
            elif chan == self.std_channel_to_raw_channel(const.BALANCES):
                continue
            ...
            for pair in subscription[chan]:
                ...
                if pair.startswith("p"):
                    ...
                    pair = self.std_symbol_to_raw_symbol(pair).lower()
    ...
    #     for data in info:
    #         update = False
    #         position = Position(self._platform, self._account, symbol=self.raw_symbol_to_std_symbol(data['symbol']))
    #         # Position.update会自动创建timestamp
    #         if not position.timestamp:
    #             update = True
    #             position.update()
    #         if Decimal(data['info']['positionAmt']) > 0:
    #             if position.long_quantity != data['contracts']:
    #                 update = True
    #                 position.update(margin_mode=MARGIN_MODE_CROSSED if data[
    #                                                                        'marginType'] == "cross" else MARGIN_MODE_FIXED,
    ...
    async def _update_position(self, data):
        ...
        """
        update = False
        position = Position(self._platform, self._account, self.raw_symbol_to_std_symbol(data['symbol']))
        if not position.timestamp:
            update = True
            position.update()
        if Decimal(data['positionAmt']) > 0:
