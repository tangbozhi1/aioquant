...
class BinanceTrade(ExchangeGateway):
    ...
    async def _get_listen_key(self):
        while True:
            try:
                key = await self._rest_api.public_post_userdatastream()
    ...
    async def _reset_listen_key(self):
        ...
        while True:
            try:
                await self._rest_api.public_put_userdatastream(self._listen_key)
    ...
    async def create_order(self, action, price, quantity, *args, **kwargs):
        ...
        client_order_id = kwargs["client_order_id"] + '_' + str(trade_type)
    ...
    async def connected_callback(self):
        if any(self.is_authenticated_channel(chan) for chan in self.websocket_channels.keys()):
            ...
            for chan in subscription:
                ...
                # 需要symbol作参数 (U本位最好不用)  websocket channel 中需要增加account chan
    ...
    #     """
    #     assets = {}
    #     for data in info['info']['balances']:
    ...
    async def _order(self, msg: dict):
        ...
        if not order:
            info = {
                ...
                "action": const.ORDER_ACTION_BUY if msg['S'] == 'BUY' else const.ORDER_ACTION_SELL,
                "order_type": const.ORDER_TYPE_LIMIT if msg['o'] == "LIMIT" else const.ORDER_TYPE_MARKET,
                "order_price_type": msg['f'],
                "ctime": msg['O'],
            }
            order = Order(**info)
            self._orders[msg['i']] = order
        order.remain = float(msg['q']) - float(msg['z'])
        order.avg_price = str(msg['p'])
        order.trade_type = const.TRADE_TYPE_NONE if len(order.client_order_id.split("_")[-1]) > 1 else \
            order.client_order_id.split("_")[-1]
        order.price = str(msg['p'])
        order.quantity = str(msg['q'])
        order.utime = msg['E']
        order.status = self.status(Decimal(msg['z']), Decimal(order.remain), Decimal(msg['q']),
                                   msg['x'])
        if order.status in [const.ORDER_STATUS_FAILED, const.ORDER_STATUS_CANCELED, const.ORDER_STATUS_FILLED]:
            self._orders.pop(msg['i'])
    ...
    async def _fill(self, data: dict):
        ...
        info = {
            ...
            "action": const.ORDER_ACTION_BUY if data['S'] == 'BUY' else const.ORDER_ACTION_SELL,
            "price": str(data['L']),
            "quantity": str(data['l']),
            "role": const.LIQUIDITY_TYPE_TAKER if data['m'] == False else const.LIQUIDITY_TYPE_MAKER,
            "fee": data['n'] + "_" + data['N'] if data.get('N') is not None else None,
            "ctime": data['T']
        }
        fill = Fill(**info)
...
class BinanceMarket(ExchangeWebsocket):
    ...
    async def _address(self):
        ...
        for chan in subscription:
            ...
            # 私有在公共线路无用，过滤(过滤掉websocket channel 中有并且自定义的和不需要的chan
    ...
    async def _book(self, msg: dict, pair: str):
        ...
        delta = {"bids": [], "asks": []}
        self._orderbooks[pair].timestamp = msg["E"]  # 无论是否reset,都需要更新
        for side in ('b', 'a'):
